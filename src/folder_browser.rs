...
pub fn browse(path: &str) -> Result<serde_json::Value> {
    ...
    if path.trim().is_empty() {
        return Ok(
            serde_json::json!({ "path": null, "parent": null, "folders": roots, "roots": roots }),
        );
    }
    ...
    for entry in std::fs::read_dir(&path)? {
        ...
        // not block browsing their parent. A link can still be opened by path.
...
mod tests {
    ...
    fn empty_path_lists_filesystem_roots() {
        ...
        assert_eq!(listing["folders"], listing["roots"]);
        assert!(!listing["roots"].as_array().unwrap().is_empty());
        #[cfg(not(windows))]
        assert_eq!(listing["roots"][0]["path"], "/");
    }

...
export function NewProjectForm({
  ...
  // null derives the subfolder name from the project name; a string is the user's edit.
          ...
          {mode === "paper" ? (
            ...
            <>
              ...
              >
                ...
                <span className={path ? "text-sm" : "placeholder"}>
                  {path || m.new_project_choose_existing_folder()}
                </span>
            ...
            <div className="flex flex-col gap-3">
              <div className="project-location-field">
                <span className="project-location-label">{m.new_project_parent_folder()}</span>
                <button type="button" className="folder-picker-control" disabled={pickingFolder}
                  aria-label={m.new_project_choose_parent_folder()}
                  title={blankParentPath}
                  onClick={() => { setPickingFolder(true); setError(null); }}>
                  <FolderOpen className="folder-picker-icon" size={16} />
                  <span className="min-w-0 flex-1 truncate text-left text-sm" dir="ltr">{blankParentPath}</span>
                  <ChevronRight className="folder-picker-chevron" size={15} />
                </button>
              </div>
              ...
              <label className="project-location-field">
                <span className="project-location-label">{m.new_project_child_folder_name()}</span>
                <input className="text-sm font-normal" value={childFolderName}
                  onChange={(event) => setBlankFolderName(event.target.value.trim() ? event.target.value : null)}
              ...
              <div className="project-location-field">
                <span className="project-location-label">{m.new_project_form_project_location()}</span>
                <span className="break-all text-sm text-subtext" dir="ltr">{projectPath || "—"}</span>
                {checkingPath && <span className="sr-only" role="status">{m.new_project_form_checking_project_location()}</span>}
                {blankDestinationHasError && <span id="blank-destination-description" className="folder-picker-hint error !text-accent-red" role="alert">

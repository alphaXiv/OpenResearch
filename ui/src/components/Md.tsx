...
function CodeBlock({ code, lang }: { code: string; lang: string | null }) {
  ...
  return (
    <div className="md-code relative my-2.5 mx-0 [&_pre]:m-0 [&:hover_.md-code-copy]:opacity-100">
      <button className="md-code-copy absolute top-1.5 end-1.5 inline-flex items-center justify-center w-6.5 h-6.5 text-muted bg-background border border-border-variant rounded-sm opacity-0 transition-[opacity,color] duration-120 ease-standard [&:hover]:text-text [&:hover]:border-muted" title={m.md_copy()} aria-label={m.md_copy_code()} onClick={copy}>

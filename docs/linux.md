...
or newer (Ubuntu 22.04, Debian 12, Fedora 36, and later) and FUSE 2 to mount
itself. Where FUSE is missing, run it with `--appimage-extract-and-run`, or set
...
variables orx cares about when it starts. It uses port 4792, or a free port if
...
| Error dialogs | If the app can't start, it says why on stderr only; run the AppImage from a terminal to see it. |
...
(`openresearch-linux-<arch>-appimage`).

To build one locally on Ubuntu 22.04 or newer:

```sh
sudo apt-get install libwebkit2gtk-4.1-dev libgtk-3-dev librsvg2-dev file
